...
inline static auto strings(auto &context) noexcept(false) {
    ...
    auto const &value_ = context.cache.globals.engine->pStringBase;
...
inline static auto clients_limit(auto const &context) noexcept(false) {
    ...
    auto const &value_ = context.cache.globals.engine->maxClients;
...
inline static auto entities_limit(auto const &context) noexcept(false) {
    ...
    auto const &value_ = context.cache.globals.engine->maxEntities;
...
inline static auto entity(auto const &context, auto index) noexcept(false) {
    ...
    if (
        driver_.pEntityOfEntityIndex != context.cache.entity
...
this_::Type::Type(Api const &api) noexcept(false):
    context_{::std::decay_t<decltype(*context_)>{.driver{api}}}
{
    try {
        auto &context_ = *(this->context_);

        if (! (
            context_.cache.globals.meta = api.meta.globals
        )) throw ::std::invalid_argument{"empty meta.globals pointer"};

        if (! (
            context_.cache.globals.engine = api.engine.globals
        ...
        if (! (
            context_.cache.entity = engine_functions_.pEntityOfEntityIndex
