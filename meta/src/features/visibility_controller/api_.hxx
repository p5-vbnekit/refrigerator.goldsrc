...
using Driver = parent_::parent_::parent_::Api;
...
private:
    struct Context_ final {
        ...
        struct Cache final {
            ...
            ::std::decay_t<
                decltype(globals.engine->pStringBase)
            > strings = nullptr;

            ::std::decay_t<decltype(
                driver.engine.functions.pEntityOfEntityIndex
            )> entity = nullptr;

            ::std::decay_t<decltype(
                globals.engine->maxClients
            )> clients_limit = 0;

            ::std::decay_t<decltype(
                globals.engine->maxEntities
            )> entities_limit = 0;
        } mutable cache = {};
    };
