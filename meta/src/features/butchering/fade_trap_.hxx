...
private:
    using Solid_ = ::std::decay_t<decltype(
        ::std::declval<Pointer>()->v.solid
    )>;

    using RenderMode_ = ::std::decay_t<decltype(
        ::std::declval<Pointer>()->v.rendermode
    )>;

    using RenderAmount_ = ::std::decay_t<decltype(
        ::std::declval<Pointer>()->v.renderamt
    )>;
    ...
    mutable ::p5::lambda::utils::event::Dispatcher<Pointer> dispatcher_ = {};
